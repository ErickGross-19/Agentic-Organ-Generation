...
def _point_to_segment_distance(point: np.ndarray, seg_start: np.ndarray, seg_end: np.ndarray) -> float:
    ...
    if seg_len_sq < 1e-12:  # Degenerate segment
...
class SpaceColonizationParams:
    ...
    use_dynamic_spatial_index: bool = True  # Use DynamicSpatialIndex instead of KD-tree (backward compatible)
    spatial_index_cell_size: Optional[float] = None  # Auto-compute if None (2x influence_radius)
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "use_dynamic_spatial_index": self.use_dynamic_spatial_index,
            "spatial_index_cell_size": self.spatial_index_cell_size,
        }
    ...
    def from_dict(cls, d: dict) -> "SpaceColonizationParams":
        ...
        return cls(
            ...
            use_dynamic_spatial_index=d.get("use_dynamic_spatial_index", True),
            spatial_index_cell_size=d.get("spatial_index_cell_size"),
        )
...
class SpaceColonizationState:
    ...
    # Dynamic spatial indices (NEW - for performance optimization)
    tip_spatial_index: Optional['DynamicSpatialIndex'] = None
    segment_spatial_index: Optional['DynamicSpatialIndex'] = None
    attractor_spatial_index: Optional['DynamicSpatialIndex'] = None
    spatial_index_cell_size: float = 0.001  # Auto-computed from influence_radius
...
def create_space_colonization_state(
    ...
    # Initialize spatial indices (NEW - for performance optimization)
    if params.use_dynamic_spatial_index:
        # Compute optimal cell size (2x influence_radius)
        cell_size = params.spatial_index_cell_size or (params.influence_radius * 2.0)
        state.spatial_index_cell_size = cell_size

        # Initialize indices
        state.tip_spatial_index = DynamicSpatialIndex(cell_size=cell_size)
        state.segment_spatial_index = DynamicSpatialIndex(cell_size=cell_size)
        state.attractor_spatial_index = DynamicSpatialIndex(cell_size=cell_size)

        # Pre-populate tips (as degenerate segments for point queries)
        eps = np.array([1e-9, 0, 0])
        for tip_id in state.active_tip_ids:
            if tip_id in network.nodes:
                node = network.nodes[tip_id]
                pos = np.array([node.position.x, node.position.y, node.position.z])
                state.tip_spatial_index.add_segment(
                    start=pos,
                    end=pos + eps,
                    radius=0.0
                )
        ...
        for seg_id, seg in network.segments.items():
            ...
            state.segment_spatial_index.insert_segment(
                segment_id=seg_id,
                start=start_pos,
                end=end_pos,
                radius=seg.geometry.radius_start
            )

        # Pre-populate attractors (tissue points as degenerate segments)
        for idx, tp_pos in enumerate(tissue_points_arr):
            state.attractor_spatial_index.insert_segment(
                segment_id=idx,
                start=tp_pos,
                end=tp_pos + eps,
                radius=0.0
        ...
        # Legacy KD-tree path (backward compatibility)
...
def space_colonization_one_step(
    ...
    # Attractor-tip association: NEW spatial index path vs legacy KD-tree path
    attractions: Dict[int, List[int]] = {tid: [] for tid in state.active_tip_ids}

    if state.attractor_spatial_index is not None and state.tip_spatial_index is not None:
        # NEW: Reverse query - iterate tips (fewer), find attractors (many)
        # This is O(k * m_local) instead of O(m * log k) where k = tips, m = attractors
        eps = np.array([1e-9, 0, 0])
        ...
        for tip_id in state.active_tip_ids:
            ...
            node = network.nodes[tip_id]
            tip_pos = np.array([node.position.x, node.position.y, node.position.z])

            # Find attractors within influence_radius of this tip
            candidate_attr_ids = state.attractor_spatial_index.query_candidate_segments_for_capsule(
                start=tip_pos,
                end=tip_pos + eps,
                radius=params.influence_radius
            )

            for attr_id in candidate_attr_ids:
                if attr_id not in state.active_tissue_indices:
                    continue

                attr_pos = state.tissue_points[attr_id]
                distance = np.linalg.norm(tip_pos - attr_pos)

                if distance <= params.influence_radius:
                    attractions[tip_id].append(attr_id)
    else:
        # Legacy KD-tree path (backward compatibility)
                    ...
                    if rng.random() < params.bifurcation_probability:
                        ...
                        for cluster_idx, cluster in enumerate(clusters):
                            ...
                            if branch_result.is_success():
                                ...
                                new_segment_id = branch_result.new_ids["segment"]
                                new_node_ids.append(new_node_id)
                                new_segment_ids.append(new_segment_id)
                                grown_any = True
                                children_created += 1

                                state.active_tip_ids.add(new_node_id)

                                # Update spatial indices incrementally (NEW)
                                if state.tip_spatial_index is not None:
                                    new_node = network.nodes[new_node_id]
                                    new_pos = np.array([new_node.position.x, new_node.position.y, new_node.position.z])
                                    eps = np.array([1e-9, 0, 0])
                                    state.tip_spatial_index.add_segment(
                                        start=new_pos,
                                        end=new_pos + eps,
                                ...
                                if state.segment_spatial_index is not None and new_segment_id in network.segments:
                                    new_seg = network.segments[new_segment_id]
                                    start_node = network.nodes[new_seg.start_node_id]
                                    end_node = network.nodes[new_seg.end_node_id]
                                    start_pos = np.array([start_node.position.x, start_node.position.y, start_node.position.z])
                                    end_pos = np.array([end_node.position.x, end_node.position.y, end_node.position.z])
                                    state.segment_spatial_index.insert_segment(
                                        ...
                                        radius=new_seg.geometry.radius_start
                                    )
    ...
    if new_node_ids:
        ...
        # Tissue point killing: NEW spatial index path vs legacy KD-tree path
        attractors_killed = 0

        if state.segment_spatial_index is not None and state.attractor_spatial_index is not None:
            # NEW: For each new segment only, kill nearby attractors
            # This is O(k * m_local) instead of O(m * log n) where k = new segments
            for new_seg_id in new_segment_ids:
                if new_seg_id not in network.segments:
                    continue

                seg = network.segments[new_seg_id]
                start_node = network.nodes[seg.start_node_id]
                end_node = network.nodes[seg.end_node_id]
                start_pos = np.array([start_node.position.x, start_node.position.y, start_node.position.z])
                end_pos = np.array([end_node.position.x, end_node.position.y, end_node.position.z])

                # Find attractors within kill_radius of this segment
                eps = np.array([1e-9, 0, 0])
                nearby_attr_ids = state.attractor_spatial_index.query_candidate_segments_for_capsule(
                    start=start_pos,
                    end=end_pos,
                    radius=params.kill_radius
                )

                for attr_id in nearby_attr_ids:
                    if attr_id not in state.active_tissue_indices:
                        continue

                    attr_pos = state.tissue_points[attr_id]

                    # Compute exact distance to segment
                    dist = _point_to_segment_distance(attr_pos, start_pos, end_pos)
                    if dist <= params.kill_radius:
                        state.active_tissue_indices.discard(attr_id)
                        attractors_killed += 1
        else:
            # Legacy KD-tree path (backward compatibility)
