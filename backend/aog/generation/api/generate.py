...
def _generate_scaffold_topdown(
    ...
    collision_online = CollisionOnlineConfig(
        ...
        merge_on_collision=collision_online_params.get('merge_on_collision', False),
        merge_distance_m=collision_online_params.get('merge_distance_m', 0.0002),
        merge_prefer_same_inlet=collision_online_params.get('merge_prefer_same_inlet', True),
        fail_retry_rounds=collision_online_params.get('fail_retry_rounds', 0),
        fail_retry_mode=collision_online_params.get('fail_retry_mode', 'both'),
        fail_retry_shrink_factor=collision_online_params.get('fail_retry_shrink_factor', 0.85),
        fail_retry_step_boost=collision_online_params.get('fail_retry_step_boost', 1.2),
    )
    ...
    config = ScaffoldTopDownConfig(
        ...
        boundary_extra_m=backend_params.get('boundary_extra_m', 0.0),
        bottom_zone_height_m=backend_params.get('bottom_zone_height_m', 0.0003),
        bottom_spread_scale_min=backend_params.get('bottom_spread_scale_min', 0.2),
        stop_before_boundary_m=backend_params.get('stop_before_boundary_m', 0.0001),
        stop_before_boundary_extra_m=backend_params.get('stop_before_boundary_extra_m', 0.0001),
        clamp_mode=backend_params.get('clamp_mode', 'shorten_step'),
        depth_adaptive_mode=backend_params.get('depth_adaptive_mode', 'none'),
        branch_plane_mode=backend_params.get('branch_plane_mode', 'global'),
        branch_plane_blend=backend_params.get('branch_plane_blend', 0.5),
